...
generated, never hand-authored. One JSON object **per SNC notification** (the `feed`
granularity — lets all three targets check `snc_ts` via `stats().last_device_time_us`):
```json
{ "snc_ts": 8534162, "samples": [[u, m, r], [u, m, r], ...] }
```
`snc_ts` = the notification's uint32 device-clock µs trailer; each `samples` entry is a
decoded int32 `[ulnar, median, radial]` (18 per notification). Parity: native/python/wasm
must all reproduce this bit-exact. Generated by the native parity test
(`MUDRAKA_REGEN_GOLDEN=1`, see `TEST_STRATEGY.md`).
